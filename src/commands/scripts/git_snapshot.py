...
payload = {
    ...
    "head_subject": head_subject["stdout"].strip(),
    "origin_url": origin["stdout"].strip() if origin["returncode"] == 0 else None,
    "origin_error": None if origin["returncode"] == 0 else origin["stderr"].strip(),
    ...
    "changed_files": split_lines(name_only["stdout"]),
    "staged_changed_files": split_lines(staged_name_only["stdout"]),
